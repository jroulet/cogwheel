#!/usr/bin/env bash
# Resume one Codex driver thread after a terminal build event or escalation.
...
EVENT="$1"
DETAIL="$2"
EVENT_ID="${3:-${CODEX_BUILD_EVENT_ID:-}}"
THREAD_ID="${CODEX_THREAD_ID:-}"

if [[ -z "$THREAD_ID" ]] || [[ "${CODEX_EVENT_RESUME:-1}" == "0" ]]; then
  exit 0
fi
if [[ ! "$THREAD_ID" =~ ^[A-Za-z0-9._:-]+$ ]]; then
  echo "Invalid CODEX_THREAD_ID; refusing callback." >&2
  exit 65
fi

REPO_ROOT="$(git -C "$(dirname "$0")" rev-parse --show-toplevel)"
STATE_DIR="${CODEX_RESUME_STATE_DIR:-/tmp/cogwheel-codex-resume}"
LOG_FILE="${CODEX_RESUME_LOG:-$STATE_DIR/resume.log}"
mkdir -p "$STATE_DIR"
...
THREAD_KEY="$(printf '%s' "$THREAD_ID" | sha256sum | awk '{print $1}')"
EVENT_KEY="$(printf '%s\0%s\0%s' "$EVENT" "$DETAIL" "$EVENT_ID" \
  | sha256sum | awk '{print $1}')"
LOCK_FILE="$STATE_DIR/thread-$THREAD_KEY.lock"
DONE_FILE="$STATE_DIR/thread-$THREAD_KEY-event-$EVENT_KEY.done"
...
IFS=',' read -r -a RETRY_DELAYS \
  <<< "${CODEX_RESUME_RETRY_DELAYS:-0,30,120}"
for DELAY in "${RETRY_DELAYS[@]}"; do
  if [[ ! "$DELAY" =~ ^[0-9]+$ ]]; then
    echo "Invalid CODEX_RESUME_RETRY_DELAYS entry: $DELAY" >&2
    exit 66
  fi
  (( DELAY > 0 )) && sleep "$DELAY"
  {
    printf '\n[%s] event=%s event_id=%s thread=%s\n' \
      "$(date --iso-8601=seconds)" "$EVENT" "$EVENT_ID" "$THREAD_ID"
    cd "$REPO_ROOT"
