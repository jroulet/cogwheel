...
| 2 | **Spec + data contracts as checkable invariants** | `SPEC.md` and (if present) `.claude/spec/DATA_CONTRACTS.yaml` are **sources of truth** — not just descriptions of the code, but constraints on it. Any artifact (code, documentation, visualization) that touches something the spec or contracts define must be consistent with them. Verification is **bidirectional** and you own the accuracy of this invariant: <br>**Spec/contracts → artifact**: If the spec or contracts make a claim (a producer/consumer relationship, a stage boundary, a module responsibility, a data format, a convention), any artifact that represents or implements that claim must match. Inconsistency is a bug in the artifact. <br>**Artifact → spec/contracts**: If an artifact introduces something not described in the spec or contracts (a new data path, a new consumer, a changed convention, a cross-lane dependency), flag the inconsistency. It could be a legitimate addition (spec/contracts need updating) or a bug in the artifact (the spec/contracts are correct and the artifact is wrong). Do not assume which — report the finding with both interpretations so it can be triaged upstream. <br>**Conventions**: Check any `DATA_CONTRACTS.yaml` conventions relevant to symbols that appear in the diff. The convention names evolve with the YAML — read the file, don't rely on a memorized list. <br>**Consumer impact**: for a changed artifact schema, run `python scripts/pipeline_graph.py consumers_of <artifact>` to confirm every pre-existing consumer still handles it. The graph reflects PRE-DIFF state — use it to check existing consumers; for consumers the diff ADDS, verify from the diff itself and do NOT flag a newly-added consumer as "missing from the graph". |
| 3 | **Module boundaries** | Read SPEC.md's layer/module descriptions. Functions must live where the spec says. Use `find_symbol` and `find_referencing_symbols` to verify — if a function is imported from the wrong module, that's misplaced logic. |

### Implementation correctness

| # | Check | How |
|---|-------|-----|
| 4 | **Goal achieved** | Cross-check what was asked vs what actually changed. Flag planned work with no diff. |
| 5 | **Caller/callee consistency** | Use `find_referencing_symbols` to trace ALL call sites of changed functions. Verify argument order, types, and count match the definition. Do NOT grep — use semantic tools. |
| 5b | **Existing tests survive the change (READ, don't run)** | Whenever the diff changes a signature, a constant's value or meaning, a coordinate/gauge definition, a tag/schema string, or the DOMAIN the code serves (a new admission threshold, refusal predicate, floor, or gate term), search ALL of `cogwheel/tests/` for that symbol and read each hit. **SKIPPED AND GATED TESTS COUNT** — a test behind `skipUnless(COGWHEEL_TRAIN_TIER)`, `@expectedFailure`, or any skip did not run in this build and will not report its own breakage. A call site missing a newly-required argument, an assertion on a bumped schema string, or a fixture built in the retired coordinate system is a finding even though the suite was green. Do NOT run expensive tests to check — that is the driver's post-build job; read them. (2026-07-27: a coordinate migration plus an admission change left 25 tests erroring at setup, undetected for a build cycle, because every one was in a file nobody ran.) <br>**A GREEN test can be the finding.** When the diff adds a domain restriction, check every pre-existing fixture against it: one that now sits OUTSIDE the served domain still compiles, still passes, and certifies nothing — it measures the code where production never goes. Flag it, and flag its tolerance with it, because a bar calibrated outside the domain is usually far looser than the domain supports and will be read as the code's real accuracy. (2026-08-13: a `rho = 1.5` saddle fixture survived a `rho >= 2.0` serve floor, advertising a 5e-3 accuracy bar where the served domain supports 2e-4.) |
| 6 | **Import correctness** | Check for explicit layer paths per CLAUDE.md import convention. Run `python -c "import <module>"` to verify. |
| 7 | **Edge cases** | Empty collections, off-by-one errors, boundary conditions, unit mismatches, NaN/null propagation, dict ordering assumptions. - **Units & conventions**: frequencies in Hz, times in GPS seconds, component masses in solar
