...
loads the artifact, calls `run`, prints the per-route breakdown and dumps
...
def _print_breakdown(report: dict) -> None:
    """Print the per-route breakdown and the residual 3-way split.

    The route rows are driven off ``report['route_counts']`` so any change to
    `serve_route_census.SERVE_ROUTES` (e.g. the ``born_carrier_only`` split)
    appears automatically -- no label count is hard-coded here.
    """
