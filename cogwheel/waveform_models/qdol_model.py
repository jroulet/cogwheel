...
from cogwheel.waveform_models.qdol_phase import gen_h0_qdol_phase
...
def compute_hplus_hcross_quadrupole_dissipation_octupole_love(
    ...
    wf_params = np.array([Mc, eta, par_dic['s1z'], par_dic['s2z'], par_dic['kappa1'], par_dic['kappa2'], 
                          ...
                          par_dic['d_luminosity'], 0, par_dic['phi_ref']]) # tc in gen_h0_qdol_phase set to zero
    ...
    hplus, hcross = waveform.compute_hplus_hcross(f,par_dic,'IMRPhenomD')
    qdol_phase = gen_h0_qdol_phase(f,wf_params,f_ref)
    hplus, hcross = np.array(hplus) * np.exp(-1.0j * qdol_phase), np.array(hcross) * np.exp(-1.0j * qdol_phase)
