...
from cogwheel.waveform_models.pe_waveform_cogwheel_qdl import gen_taylorF2_qdl_polar


DEFAULT_PARS = waveform.DEFAULT_PARS | {'h1': 1., 'h2': 1.} | {'kappa1': 1., 'kappa2': 1.}


def compute_hplus_hcross_quadrupole_dissipation_love(
        f, par_dic, approximant='quadrupole_dissipation_love',
        harmonic_modes=None, lal_dic=None):
    """
    Generate frequency domain waveform with spin-induced quadrupole, 
    tidal dissipation and tidal deformability parameters.
    ...
    """
    del lal_dic
    if approximant != 'quadrupole_dissipation_love':
        raise ValueError('`approximant` must be "quadrupole_dissipation_love"`')
    ...
    wf_params = np.array([Mc, eta, par_dic['s1z'], par_dic['s2z'], par_dic['kappa1'], par_dic['kappa2'], 
                          par_dic['h1'], par_dic['h2'], par_dic['l1'], par_dic['l2'],
                          par_dic['d_luminosity'], 0, par_dic['phi_ref'], par_dic['iota']]) # tc in gen_taylorF2_qdl_polar set to zero
    f_ref = par_dic['f_ref'] 
    ...
    hplus, hcross = gen_taylorF2_qdl_polar(f, wf_params, f_ref)
    hplus, hcross = np.array(hplus), np.array(hcross)
    hplus_hcross = np.stack([hplus, hcross])    
    return hplus_hcross
    

waveform.APPROXIMANTS['quadrupole_dissipation_love'] = waveform.Approximant(
    hplus_hcross_func=compute_hplus_hcross_quadrupole_dissipation_love)


class QuadrupoleDissipationLoveWaveformGenerator(waveform.WaveformGenerator):
    """
    Similar to WaveformGenerator but expects tidal dissipation
    parameters in addition.
    """
    params = sorted(waveform.WaveformGenerator.params + ['kappa1', 'kappa2', 'h1', 'h2'])


class QuadrupoleDissipationLoveMarginalizedExtrinsicLikelihoodQAS(
    ...
    """
    params = sorted(MarginalizedExtrinsicLikelihoodQAS.params + ['kappa1', 'kappa2', 'h1', 'h2'])
    ...
    def from_reference_waveform_finder(
        ...
        **kwargs:
        ...
        Instance of ``cls``.
        """
        waveform_generator = QuadrupoleDissipationLoveWaveformGenerator(
...
class IntrinsicQuadrupoleDissipationLovePrior(RegisteredPriorMixin,
                                      CombinedPrior):
    """
    Prior for usage with
    ``QuadrupoleDissipationLoveMarginalizedExtrinsicLikelihoodQAS``.
