...
# theta ordering: Mc, eta, chi_1, chi_2, h1, h2, Love1, Love2, dL, tc, phic, iota
...
def Phif3hPN_TLN(f, theta):
    ...
    """
    Mc, eta, chi_1, chi_2, h1, h2, Love1, Love2 = theta # TODO: Old version takes in M, not Mc # TODO: Include H
    M = Mc / (eta ** (3 / 5))
...
def gen_h0(f, theta, f_ref):
    ...
    Strain (array):
    """
    Mc, eta, _, _, _, _, _, _, Deff, tc, phic = theta # h1, h2, Love1, Love2
    M = Mc / (eta ** (3 / 5))
    ...
    # f_phasecutoff = f[np.argmax(grad_phase_fcut)]
    # f_ISCO = 4.4e3 * (1 / M)  # Hz
    ...
    f_ISCO = 4.4e3 * (1 / M)  # Hz
    f_cutoff = np.min(np.array([f_ISCO, f_phasecutoff]))
    # print('grad_phase:', grad_phase)
    # print('f_phasecutoff:', f_phasecutoff)
    ...
    # # Jax version
    # Amp_m = Amp_merger(f, f_cutoff, A0[np.argmax(grad_phase_fcut)])
    ...
    return Amp * pre * np.exp(-1.0j * Phi) + 1e-30 # LOVE: Add tiny value to avoid zero division in finding reference wf

def gen_taylorF2_dissipation_polar(f, params, f_ref):
    """
    ...
    """
    # iota = params[9] # This is when there is only Love numbers
    # iota = params[11] # When there is h1, h2, l1, l2
    iota = params[-1]
...
def custom_compute_hplus_hcross(f, par_dic, *_):
    ...
    # wf_params ordering: ['Mc', 'eta', 'chi_1', 'chi_2', 'Love1', 'Love2', 'Deff', 'tc', 'phic', 'iota', 'f_ref']
    
    eta = par_dic['m1']*par_dic['m2'] / (par_dic['m1']+par_dic['m2'])**2
    Mc = (par_dic['m1']+par_dic['m2'])*eta**(3/5)
    
    # TODO: Augment h1 h2 into cogwheel 
    ...
    # TODO LOVE: Add to ripple to avoid divergence in amplitude at f=0
    f0_is_0 = f[0] == 0 
    if f0_is_0:
...
# warnings.warn('Overwriting the code of ``waveform.compute_hplus_hcross`` '
# 	          f'with that of {__name__}.custom_compute_hplus_hcross. '
# 	          'To undo, either reload ``waveform.py`` or restart the kernel.',
# 	          stacklevel=2)
# # waveform.compute_hplus_hcross.__code__ = custom_compute_hplus_hcross.__code__ # This doesn't work
# waveform.compute_hplus_hcross = custom_compute_hplus_hcross
