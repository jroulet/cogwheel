...
class MarginalizedRelativeBinningLikelihood(RelativeBinningLikelihood):
    ...
    def lnlike_no_marginalization_from_timeseries(
        ...
        ndet = len(self.event_data.detector_names)
        zs = np.zeros(ndet, dtype=np.complex128)
        ts = np.zeros(ndet, dtype=np.complex128)
        for ind_det, det in enumerate(self.event_data.detector_names):
    ...
    def generate_extrinsic_params_from_cs(
        ...
        dt_1, = gw_utils.time_delay_from_geocenter(
            self.event_data.detector_names[0], ra, dec, self.event_data.tgps)
