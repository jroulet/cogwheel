...
An experimental layer, :py:mod:`cogwheel.lensing.chang_refsdal`, provides wave-optics amplification for microlensed gravitational-wave signals under a Chang–Refsdal lens (a point mass embedded in external convergence and shear). It couples a double-double arithmetic substrate and image geometry (quartic image solver, Fermat delays, magnifications, stationary-phase kernels) to a contour-free amplification operator and a topology-stable channel tracker (:py:class:`~cogwheel.lensing.chang_refsdal.ChangRefsdalChannels`, the public entry point) that decomposes the amplification into delay-labelled channels. The engine is certified for both macro-image parities: positive parity (``1 - kappa > |gamma|``) up to dimensionless frequency ``w <= 500`` on the operator wave branch, and the macro saddle (``1 - kappa < |gamma|``) up to ``w <= 60`` on the Schwinger branch. The parity wall itself, ``1 - kappa == |gamma|`` (``det A = 0``, where the macro magnification diverges), is a measure-zero named refusal.

Two higher-level entry points build on this engine. :py:class:`~cogwheel.lensing.waveform.LensedWaveformGenerator` wraps an ordinary :py:class:`~cogwheel.waveform.WaveformGenerator` and multiplies every harmonic mode by the shared amplification factor ``F(w(f))``, where the dimensionless frequency ``w = 8 * pi * G * M_L * (1 + z_L) * f / c**3`` is linear in the observed frequency ``f`` (``M_L`` is the redshifted lens mass in solar masses, ``z_L`` the lens redshift). It exposes the per-image ``(tau_a, K_a)`` channel decomposition, not only the collapsed total. :py:class:`~cogwheel.lensing.likelihood.LensedRelativeBinningLikelihood` then evaluates a fast `relative binning <https://arxiv.org/abs/1806.08792>`_ log-likelihood for microlensed signals, heterodyning against an *unlensed* reference waveform and contracting delay-continuous frequency-moment summaries mode-then-image (image-delay phases kept analytic, kernels interpolated). Both are experimental and cover both parities. A configuration exactly on the parity wall, ``1 - kappa == |gamma|``, raises :py:exc:`~cogwheel.lensing.chang_refsdal.geometry.LensDomainError` at the API boundary rather than returning a degraded result; the same named-refusal contract applies wherever the engine cannot certify a result, so a refusal is always explicit rather than a silently degraded number.
